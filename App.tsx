...
import { parseLrc, extractColors, parseAudioMetadata, parseNeteaseLink, shuffleArray } from './services/utils';
import { searchAndMatchLyrics, fetchNeteasePlaylist, fetchNeteaseSong, getNeteaseAudioUrl, fetchLyricsById } from './services/lyricsService';
...
const App: React.FC = () => {
  ...
  const [matchStatus, setMatchStatus] = useState<'idle' | 'matching' | 'success' | 'failed'>('idle');
  ...
  useEffect(() => {
      if (currentSong && currentSong.isNetease && (!currentSong.lyrics || currentSong.lyrics.length === 0) && matchStatus === 'idle') {
      ...
      else if (currentSong && !currentSong.isNetease && (!currentSong.lyrics || currentSong.lyrics.length === 0) && matchStatus === 'idle') {
          const match = async () => {
            setMatchStatus('matching');
            const result = await searchAndMatchLyrics(currentSong.title, currentSong.artist);
            if (result) {
  ...
  useEffect(() => {
      if (currentSong && currentSong.isNetease && currentSong.coverUrl && (!currentSong.colors || currentSong.colors.length === 0)) {
  ...
  const updateSongInQueue = (id: string, updates: Partial<Song>) => {
      setQueue(prev => prev.map(s => s.id === id ? { ...s, ...updates } : s));
      setOriginalQueue(prev => prev.map(s => s.id === id ? { ...s, ...updates } : s));
  };
  ...
  const handleImportUrl = async (input: string) => {
      ...
      if (!parsed) {
          alert("Invalid Netease URL. Use https://music.163.com/#/song?id=... or playlist");
          return;
      }
  ...
  return (
    <div className="relative w-full h-screen flex flex-col overflow-hidden">
      ...
      <div className="fixed top-0 left-0 w-full h-14 z-[60] group">
        ...
        <div className="relative z-10 w-full h-full px-6 flex justify-between items-center opacity-0 group-hover:opacity-100 translate-y-[-10px] group-hover:translate-y-0 transition-all duration-500 delay-75">
            <div className="flex items-center gap-4">
                <h1 className="text-white/90 font-bold tracking-wider text-sm uppercase">Aura Music</h1>
            </div>
      ...
      {currentSong && <GeminiButton song={currentSong} />}
    </div>
  );
};
