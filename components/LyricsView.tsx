...
const LyricsView: React.FC<LyricsViewProps> = ({ lyrics, audioRef, isPlaying, currentTime, onSeekRequest, matchStatus }) => {
    ...
    useLayoutEffect(() => {
        const loop = (now: number) => {
            ...
            const isUserInteracting = sState.isDragging || timeSinceInteraction < RESUME_DELAY_MS;
            ...
            if (isUserInteracting) {
                ...
                system.setTarget('y', targetY, { mass: 1, stiffness: 80, damping: 20, precision: 0.1 });
            }
            ...
            if (containerRef.current) {
                ...
                lineRefs.current.forEach((lineEl, index) => {
                    ...
                    // Opacity: Significantly lower for non-highlighted lines to reduce visual noise
                    // If user is scrolling manually, keep opacity slightly higher to read
                    const minOpacity = sState.visualState ? 0.4 : 0.15; 
                    const opacity = 1.0 - Math.pow(normDist, 0.5) * (1.0 - minOpacity);
                    ...
                    lineEl.style.filter = blur > 0.5 ? `blur(${blur.toFixed(1)}px)` : 'none';
                });
            }
